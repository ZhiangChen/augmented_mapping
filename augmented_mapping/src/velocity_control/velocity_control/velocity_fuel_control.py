...
class VelocityControl(Node):
   ...
   def __init__(self) -> None:
       ...
       self.trigger_subscriber = self.create_subscription(Empty, 'state_machine/velocity_control', self.trigger_callback, 10)
   ...
   def timer_callback(self) -> None:
       ...
       if self.takeoff == True:
           if self.vehicle_local_position.z < -2.5 and self.trigger:
               self.takeoff = False
